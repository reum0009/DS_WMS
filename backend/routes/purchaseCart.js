...
function documentCategoryForItem(item) {
  ...
  if (SOFTWARE_EXPENSE_MARKERS.some(marker => text.includes(marker))) return '비용';
  if (SOFTWARE_ASSET_MARKERS.some(marker => text.includes(marker))) return '컴퓨터소프트웨어';
  if (FIXTURE_MARKERS.filter(marker => marker !== 'pc').some(marker => text.includes(marker))) return '집기비품';
  if (CONSUMABLE_MARKERS.some(marker => text.includes(marker))) return '소모품';
