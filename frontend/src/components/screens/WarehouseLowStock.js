...
const RED='#f85149',GRN='#3fb950',YLW='#e3b341',ORG='#f0883e';
...
const WarehouseLowStock = ({user, onGoHome}) => {
  ...
  const lowCnt = products.filter(p => (p.currentStock || 0) > 0).length;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: BG0, color: TX, fontFamily: "'Noto Sans KR', sans-serif", overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 15, padding: '20px 32px', background: BG1, borderBottom: `2px solid ${BD2}`, flexShrink: 0 }}>
        {[
          { label: '소진 품목', value: emptyCnt + '종', color: RED, bg: '#2d0d0b', icon: '🔴' },
          { label: '부족 품목', value: lowCnt + '종', color: YLW, bg: '#2d2000', icon: '⚠️' },
          { label: '발주 필요', value: products.length + '종', color: ORG, bg: '#2d1800', icon: '📦' },
          { label: '발주 완료', value: ordered.size + '종', color: GRN, bg: '#0d2616', icon: '✅' },
        ].map(c => (
          <div key={c.label} style={{ flex: 1, background: c.bg, border: `2px solid ${c.color}44`, borderRadius: 15, padding: '15px', textAlign: 'center' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '50px 1fr 150px 120px 120px 120px', background: BG2, borderBottom: `2px solid ${BD}`, padding: '0 20px', flexShrink: 0 }}>
            {['#', '품목명', '현재고 / 안전', '부족량', '단가', '액션'].map((h, i) => (
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {loading ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200, color: TX3, fontSize: 16 }}>저재고 품목이 없습니다.</div>
            ) : sorted.map((p, i) => {
              const aStock = p.currentStock || 0;
              const aSafety = p.safetyStock || 0;
              const isEmpty = aStock === 0;
              const shortfall = aSafety - aStock;
              const shortageText = isEmpty ? '전량소진' : (shortfall > 0 ? `-${shortfall}${p.unit}` : '저재고');
              const pct = Math.min(100, aSafety > 0 ? Math.round(aStock / aSafety * 100) : 0);
              const color = isEmpty ? RED : YLW;
              const isOrdered = ordered.has(p.id);
              const isSel = p.id === selId;
              return (
                <div key={p.id} onClick={() => setSelId(isSel ? null : p.id)}
                  style={{ display: 'grid', gridTemplateColumns: '50px 1fr 150px 120px 120px 120px', padding: '0 20px', borderBottom: `2px solid ${BD2}`, cursor: 'pointer',
                  ...
                  <div style={{ padding: '25px 5px', textAlign: 'center' }}>
                    <span style={{ fontSize: 20, fontWeight: 900, color: color }}>{aStock}</span>
                    <span style={{ fontSize: 14, color: TX3, fontWeight: 600 }}> / {aSafety} {p.unit}</span>
                  </div>
                  <div style={{ padding: '25px 5px', textAlign: 'center', fontSize: 18, fontWeight: 900, color: RED }}>
                    {shortageText}
                  </div>
        ...
        {sel && (
          <div style={{ width: 340, borderLeft: `2px solid ${BD2}`, background: BG1, padding: '24px', flexShrink: 0, overflowY: 'auto' }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 900, color: TX, marginBottom: 20, lineHeight: 1.3 }}>{sel.productName}</div>
            
            <div style={{ padding: '20px', background: (sel.currentStock || 0) === 0 ? '#2d0d0b' : '#2d2000', border: `2px solid ${(sel.currentStock || 0) === 0 ? RED : YLW}33`, borderRadius: 15, marginBottom: 20, textAlign: 'center' }}>
              <div style={{ fontSize: 48, fontWeight: 900, color: (sel.currentStock || 0) === 0 ? RED : YLW }}>{sel.currentStock || 0}</div>
              ...
              <div style={{ marginTop: 15, height: 10, background: BD2, borderRadius: 5, overflow: 'hidden' }}>
                <div style={{ width: `${Math.min(100, (sel.safetyStock || 0) > 0 ? (sel.currentStock || 0) / (sel.safetyStock || 0) * 100 : 0)}%`, height: '100%', background: (sel.currentStock || 0) === 0 ? RED : YLW, borderRadius: 5 }} />
              </div>
            </div>
